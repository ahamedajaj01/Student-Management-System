
import React from 'react'

function Alert({ type = 'info', message, title,onClose }) {
    // Pure UI mapping for Tailwind classes
  const config = {
    success: {
      bg: "bg-emerald-50",
      border: "border-emerald-500",
      text: "text-emerald-800",
      iconText: "text-emerald-500",
      icon: "M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
    },
    error: {
      bg: "bg-red-50",
      border: "border-red-500",
      text: "text-red-800",
      iconText: "text-red-500",
      icon: "M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
    },
    warning: {
      bg: "bg-amber-50",
      border: "border-amber-500",
      text: "text-amber-800",
      iconText: "text-amber-500",
      icon: "M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
    },
    info: {
      bg: "bg-blue-50",
      border: "border-blue-500",
      text: "text-blue-800",
      iconText: "text-blue-500",
      icon: "M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
    }
  };

  const style = config[type] || config.info;
  return (
    <>
       <div className={`flex items-start gap-4 p-4 mb-6 border-l-4 rounded-r-2xl shadow-sm ${style.bg} ${style.border}`}>

      <div className={`mt-0.5 ${style.iconText}`}>
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
          <path fillRule="evenodd" d={style.icon} clipRule="evenodd" />
        </svg>
      </div>
      
      <div className="flex-1">
        {title && <h3 className={`text-sm font-bold mb-1 ${style.text}`}>{title}</h3>}
        <p className={`text-sm font-medium leading-relaxed ${style.text}`}>
          {message}
        </p>
      </div>
        {onClose && (
  <button
    onClick={onClose}
    className={`ml-4 rounded-md p-1 transition ${style.text} hover:bg-black/5`}
    aria-label="Close alert"
  >
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className="h-4 w-4"
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
    >
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
    </svg>
  </button>
)}
    </div>
    </>
  )
}

export default Alert
